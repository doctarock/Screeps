...
var roleBuilder = {
    spawn: function(base,population,extCount) {
        ...
        if (base.spawnCreep([WORK,WORK,CARRY,CARRY,MOVE,MOVE], newName, {memory: {role: 'repairer', action: false, home: base.room.name, away: false, target: aiSource.getSource(base,population[base.room.name].builders.length,population)}}) < 0){
            if (base.spawnCreep([WORK,CARRY,MOVE], newName, {memory: {role: 'repairer', action: false, home: base.room.name, away: false, target: aiSource.getSource(base,population[base.room.name].builders.length,population)}}) == 0){
    ...
    run: function(creep,population,iteration) {
        ...
        switch (creep.memory.action){
            ...
            case "repairing":
                creep.say('🚧');
                if (aiSource.repairAnything(creep)){
                    creep.memory.action = false;
                    creep.memory.target = false;
                };
                break;
